...
class ServiceController extends WchatController
{
    public function index($p=1){
        if(IS_AJAX){
            ...
            $pagesize = 1;
            $start=($p-1)*$pagesize;
            $list=$model->where(array('document.category_id'=>42))->join('picture ON document.cover_id = picture.id')->join('document_article ON document.id = document_article.id')->limit($start,$pagesize)->select();
